    ...
    return (
        <div className='md:flex gap-8 border-b border-gray-400 py-10' >
            ...
            <div className='md:w-[75%]'>
                ...
                <div className=' py-5 flex gap-8 md:gap-20'>
                    <div className='space-y-1 flex flex-col items-center'>
                        <img className='h-9 w-8' src={downlodeIcon} alt="" />
                        <p className='text-sm text-gray-500'>Downloads</p>
                        <h1 className='text-2xl md:text-4xl font-bold'>{appDetails.downloads}M</h1>
                    </div>
                    <div className='space-y-1 flex flex-col items-center'>
                        <img className='h-9 w-8' src={ratingIcon} alt="" />
                        <p className='text-sm text-gray-500'>Downloads</p>
                        <h1 className='text-2xl md:text-4xl font-bold'>{appDetails.ratingAvg}</h1>
                    </div>
                    <div className='space-y-1 flex flex-col items-center'>
                        <img className='h-9 w-8' src={like} alt="" />
                        <p className='text-sm text-gray-500'>Downloads</p>
                        <h1 className='text-2xl md:text-4xl font-bold'>{appDetails.reviews}K</h1>
                    </div>
                </div>
                <div>
                <button onClick={()=>handelInstall(appDetails.id)} className='btn bg-green-400 text-white font-semibold '>
                   {
                    installButton ? ` Install Now (${appDetails.size} MB)`: <p>Uninstall</p>
                   }
                </button>
            </div>
            </div>
